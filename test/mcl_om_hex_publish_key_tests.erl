...
%%% behind it. The org key this package publishes with is one, so the check
%%% refused a key that had published 0.26.1 to 0.26.3 the same day, and 0.26.4
%%% did not go out. The question publishing needs answered is whether the key
...
-module(mcl_om_hex_publish_key_tests).
...
script() ->
    filename:join([filename:dirname(?FILE), "..", "scripts", "is_hex_publish_key_live.sh"]).
